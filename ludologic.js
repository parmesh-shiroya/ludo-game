...
function distanceBetweenPlayer(currentPlayer, comparePlayer) {
    let array = [0, 1, 2, 3]
    let distance = 0;
...
for (let currentPiecePosition of currentPlayerData.piece) {
    ...
    for (let playerKey of Object.keys(players).filter(k => k != currentPlayer)) {
        ...
        for (let comparePiecePosition of comparePlayerData.piece) {
            ...
            let piecePosForCurrentPlayer = (13 * distanceBetweenPlayer(currentPlayerData.no, comparePlayerData.no)) + comparePiecePosition;
